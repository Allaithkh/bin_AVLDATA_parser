    ...
    with open(filename, 'a') as recordfile:
        ...
        try:
            ...
            # for rec in range(0, nod1):
                ...
                evt_io_id, total_io_num = unpack('!HH', raw_packet)
                (one_byte_io,) = unpack('!H', raw_packet)
                for idx in range(0, one_byte_io):
                    io_id, io_value = unpack('!HB', raw_packet)
                    parsed_io_list.append({
                        'io_id' : io_id,
                        'io_value' : io_value
                    })
                (two_byte_io,) = unpack('!H', raw_packet)
                for idx in range(0, two_byte_io):
                    io_id, io_value = unpack('!HH', raw_packet)
                    parsed_io_list.append({
                        'io_id' : io_id,
                        'io_value' : io_value
                    })
                (four_byte_io,) = unpack('!H', raw_packet)
                for idx in range(0, four_byte_io):
                    io_id, io_value = unpack('!HI', raw_packet)
                    parsed_io_list.append({
                        'io_id' : io_id,
                        'io_value' : io_value
                    })
                (eight_byte_io,) = unpack('!H', raw_packet)
                for idx in range(0, eight_byte_io):
                    io_id, io_value = unpack('!HQ', raw_packet)
                    parsed_io_list.append({
                        'io_id' : io_id,
                        'io_value' : io_value
                    })
